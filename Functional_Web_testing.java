...
//		ChromeDriver driver = (ChromeDriver) WebTesting.launchDriver("https://www.google.com", "chrome");
		...
		try {
			shortWait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//g-raised-button[jsaction=\'i0JLwd\']")));}
			catch (TimeoutException ex){
		...
		try {
			shortWait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//g-raised-button[jsaction=\'iyDKIb\']")));
		} catch (TimeoutException ex) {
			fail("Speed test failed to run.");
		}
	}
